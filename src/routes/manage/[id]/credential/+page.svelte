<script lang="ts">
	...
	// Mock class code - in real implementation, this would be fetched based on classId
	const classCode = 'ABC123';

	// Page size and pagination
	let currentPage = $state(1);
	const pageSize = 8;
	const totalPages = $derived(Math.ceil(student_list.length / pageSize));
	const paginated = $derived(
		student_list.slice((currentPage - 1) * pageSize, currentPage * pageSize)
	);
